...
class CommentRemoteManager(VkontakteTimelineManager):
    ...
    def fetch_post(self, post, offset=0, count=100, sort='asc', need_likes=True, preview_length=0, before=None, after=None, **kwargs):
        ...
        kwargs['offset'] = int(offset)
        # need_likes
        # 1 - будет возвращено дополнительное поле likes. По умолчанию поле
    ...
    @transaction.commit_on_success
    def fetch_group_post_parser(self, post, offset=0, count=None):
        '''
        Old method via parser
        '''
        post_data = {
...
#                break
        ...
        if len(items) == 20 and not need_cut:
            # , after=after, only_new=only_new)
            return self.fetch_group_post(post, offset=current_count, count=count)
    ...
    def fetch_likes(self, *args, **kwargs):
        ...
        if likes_count < self.likes:
            log.warning('Fetched ammount of like users less, than attribute `likes` of post "%s": %d < %d' % (
                self.remote_id, likes_count, self.likes))
    ...
    def fetch_reposts_api(self, *args, **kwargs):
        ...
        if reposts_count < self.reposts:
            log.warning('Fetched ammount of repost users less, than attribute `reposts` of post "%s": %d < %d' %
                        (self.remote_id, reposts_count, self.reposts))
        elif reposts_count > self.reposts:
...
class Comment(WallAbstractModel):
    ...
    # strange value, seems to be equal to author
    ...
    def parse(self, response):
        ...
        if '_' not in str(self.remote_id):
            self.remote_id = '%s_%s' % (
                self.post.remote_id.split('_')[0], self.remote_id)
...
Group.add_to_class('wall_posts', generic.GenericRelation(
    Post, content_type_field='wall_owner_content_type', object_id_field='wall_owner_id', related_name='group_wall', verbose_name=u'Сообщения на стене'))
User.add_to_class('wall_posts', generic.GenericRelation(
    Post, content_type_field='wall_owner_content_type', object_id_field='wall_owner_id', related_name='user_wall', verbose_name=u'Сообщения на стене'))
...
Group.add_to_class('comments', generic.GenericRelation(
    Comment, content_type_field='author_content_type', object_id_field='author_id', related_name='group', verbose_name=u'Комментарии'))
User.add_to_class('comments', generic.GenericRelation(
    Comment, content_type_field='author_content_type', object_id_field='author_id', related_name='user', verbose_name=u'Комментарии'))

Group.add_to_class('replies', generic.GenericRelation(
    Comment, content_type_field='reply_for_content_type', object_id_field='reply_for_id', related_name='group_wall_reply', verbose_name=u'Ответы на комментарии'))
User.add_to_class('replies', generic.GenericRelation(
    Comment, content_type_field='reply_for_content_type', object_id_field='reply_for_id', related_name='user_wall_reply', verbose_name=u'Ответы на комментарии'))
